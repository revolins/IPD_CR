...
def det_output(args):
    ...
    folder_str = folder_str + "mbits" + str(args.max_bits_of_memory)
    if args.static:
        #args.mut_rat = 0.0
...
def format_cmd(args, arg_parser):
    ...
    arg_dict.pop('max_bits_of_memory') # Have to pop values until it reaches hard_defect
    arg_dict.pop('mem_cost')
    for i in arg_dict: 
...
def run_test(args, arg_parser):
    ...
    if type(args.mem_cost) == list: args.mem_cost = args.mem_cost[0]
    mem_cost = [str(x.strip()) for x in args.mem_cost.split(',')]
    ...
    for i in range(1, args.num_test + 1):
        for cost in mem_cost:
            ...
            if args.max_bits_of_memory != 1: default_cmd.extend(['--max_bits_of_memory', str(args.max_bits_of_memory)])
            if len(temp_str) > 0: default_cmd.extend(temp_str)
            test_list.append(default_cmd)
            if args.debug_test: print("Final Command: ", default_cmd, flush=True)
    ...
    plot_str = ["python", "plot_csv.py", "-o", output_folder, "-max_m", str(args.max_bits_of_memory), "--mem_cost", str(args.mem_cost)]
    if args.hard_defect: plot_str.extend(["-hf", str(args.hard_defect)])
...
def main():
    ...
    #TODO: Merge with main? Move a lot of main functionality into utils?
    ...
    arg_parser.add_argument("--org_type", "--ot", type=str, default='pd', help="(str) (DEFAULT = pd) original memory model (pd), hybrid memory model (hybrid), summary memory model (summary), reactive tft-variant (rtft)")
    arg_parser.add_argument("--org_seed_per", "--oper", type=float, default=0.0, help="(float) (DEFAULT = 0.0) determines percentage of organisms set to special seeded-type NOTE: currently only works for reactive tft-variant organism")
    arg_parser.add_argument("--number_of_generations", "--ng", type=int, default=1000, help="(int) (DEFAULT = 1000) number of generations selected upon after a tournament")
    ...
    arg_parser.add_argument("--max_bits_of_memory", "--max_m", type=str, default='1')
